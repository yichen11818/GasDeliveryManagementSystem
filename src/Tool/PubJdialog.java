					...
					if (!InputLimit.inputIsNull(jtext_update)) {
						...
						if (isAlter_int != 0) {
							...
							if (tablePanelIndex == 0) {
								...
								String[] hintError = { "煤气ID格式错误", "煤气名格式错误", "供应商名格式出错", "价格格式错误", "库存量格式错误" };
								String message = "";
								boolean result[] = InputLimit.regular(regex, input);
								for (int i = 0; i < result.length; i++) {
									if (!result[i]) {
								...
								if (message.equals("")) {
									gasCon.updateGas(Integer.valueOf(jtext_update[1].getText().toString()),
											...
											Double.valueOf(jtext_update[4].getText().toString()),
											Integer.valueOf(jtext_update[5].getText().toString()));
